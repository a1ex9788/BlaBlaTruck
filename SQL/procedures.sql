...
BEGIN
INSERT INTO Persona(Nombre, Apellidos, DNI, Telefono, Email, Usuario, Contrase�a, IBAN, NumeroCuentaBancaria)
VALUES (@Nombre, @Apellidos, @DNI, @Telefono, @Email, @Usuario, @Contrase�a, @IBAN, @NumeroCuentaBancaria)
END
ELSE
BEGIN
UPDATE Persona SET Nombre = @Nombre, Apellidos = @Apellidos, Telefono = @Telefono, Email = @Email, Usuario = @Usuario, Contrase�a = @Contrase�a, IBAN = @IBAN, NumeroCuentaBancaria = @NumeroCuentaBancaria
WHERE DNI = @DNI
END;
